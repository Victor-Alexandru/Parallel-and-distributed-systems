...
using FuturesAndContinuations.ObjectState;
using FuturesAndContinuations.parser;

namespace FuturesAndContinuations.Implementation
{
    class AsyncTasksMechanism
    {
        ...
        private static async void StartClient(string host, int id)
        {
            ...
            var remoteEndpoint = new IPEndPoint(ipAddress, HttpParser.HTTP_PORT);
            ...
            await SendWrapper(state, HttpParser.getRequestString(state.hostname, state.endpoint));
            ...
            Console.WriteLine(
                ...
                id, HttpParser.getContentLength(state.responseContent.ToString()), state.responseContent.Length);
